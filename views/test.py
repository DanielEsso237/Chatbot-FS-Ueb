...
def load_custom_css():
    """Charge le fichier CSS pour le style de l'application."""
...
class VectorialSearchGemma:
    def __init__(self, pdf_folder, index_file="faiss_index"):
        ...
        # Le chemin vers le modèle local de sentence-transformers
        ...
        # Initialise le modèle d'embeddings LangChain avec le modèle local
        self.embeddings = HuggingFaceEmbeddings(
            model_name=self.model_path,
            model_kwargs={'device': 'cpu'} # Utilise le CPU pour éviter les problèmes de compatibilité
        )
        
        # Initialise le modèle LLM Ollama de LangChain
...
"""
    ...
    def prepare_data(self):
        """
        Charge les documents PDF, les découpe et les stocke dans la session Streamlit.
        """
        if "documents" not in st.session_state:
            st.session_state.documents = []
            
            # Utilise LangChain pour charger les PDF
    ...
    def load_index(self):
        """
        Charge l'index FAISS ou le crée s'il n'existe pas.
        """
        if os.path.exists(self.index_file):
            print("Chargement de l'index FAISS existant...")
            # Correction : Ajoutez l'argument pour permettre la désérialisation
            ...
            # Crée un nouvel index FAISS et le sauvegarde
    ...
    def create_rag_chain(self):
        """
        Construit et retourne la chaîne de RAG (Retrieval-Augmented Generation) de LangChain.
        """
        prompt = ChatPromptTemplate.from_template(self.system_prompt)
        
        # Crée la chaîne LangChain
        chain = (
    ...
    def render(self):
        """Affiche l'interface utilisateur du chatbot."""
        load_custom_css()
        self.render_header()
        self.render_sidebar()
        ...
        if user_query := st.chat_input("💬 Posez votre question..."):
            ...
            with st.chat_message("assistant"):
                with st.spinner("🤔 Recherche dans les documents..."):
                    # Exécute la chaîne RAG de LangChain
